...
function loadPolymer(href) {
	return new Promise(function(resolve, reject) {
		if (href) {
			console.log("loadPolymer");
			$("document").ready(function() {
				console.log("loadPolymer in ready");
				console.time(href);
...
var polymerPromise = (async function() {
    ...
    // .fullcalendar would not render sometimes without this delay
    await sleep(1);
})();
...
async function showToast(params) {
    ...
    $($toast[0].shadowRoot).find("#label").empty().append(params.text); // for html
...
function isFocusOnInputElement() {
	return document.activeElement.nodeName == "SELECT" || document.activeElement.nodeName == "TEXTAREA" || document.activeElement.nodeName == "INPUT" || document.activeElement.nodeName == "OVERLAY-HOST" || document.activeElement.nodeName == "PAPER-BUTTON" || document.activeElement.nodeName == "PAPER-INPUT" || document.activeElement.nodeName == "PAPER-DROPDOWN-MENU" || document.activeElement.nodeName == "PAPER-ITEM" || document.activeElement.nodeName == "PAPER-TEXTAREA" || document.activeElement.nodeName == "EVENT-REMINDERS" || document.activeElement.nodeName == "IRON-AUTOGROW-TEXTAREA" || document.activeElement.getAttribute("contenteditable");
}
