...
async function setEventDateMessage(eventEntry) {
	...
	if (eventEntry.inputSource == InputSource.QUICK_ADD) {
		...
		$message = $(`<span style='display:inline-block;min-width:100px'>${message}</span>`);
	}
...
async function showCreateBubble(params) {
	...
	function initEventEntry() {
		...
		if ($("#detectTime").is(":visible") && doesEventTitleHaveTime()) {
            ...
            const startTime = $("#eventStartTime").timepicker("getTime");
            if (startTime) {
                mergeTime(eventEntry.startTime, startTime);
            }

            if (!$("#createEventDialog").hasClass("task-selected")) {
                const endTime = $("#eventEndTime").timepicker("getTime");
                if (endTime) {
...
async function saveQuickAdd() {
	$("#save-quick-add").addClass("disabled");

    const calendarId = getDropDownValue($("#quickAddCalendarsMenu"));
...
function closeQuickAdd() {
    $("#save-quick-add").removeClass("disabled");
    ...
    if (typeof loggedOut === "undefined" || loggedOut) { // !loggedOut  - commented because issue occured when invalid creditials
        ...
        polymerPromise.then(async () => {
            if (calendarView == CalendarView.AGENDA) {
                            ...
                            if (!fcEvent.allDay && (!fcEvent.end || fcEvent.start.isSameDay(fcEvent.end))) {
                                ...
                                if (jEvent) {
                                    if (window.matchFontColorWithEventColor) {
                                        ...
                                        let html;
                                        if (needsAction) {
                                            html = `<span class='eventColorIndicator needs-action' style='border: 1px solid ${eventColors.bgColor}'>&nbsp;</span>`;
                                        } else {
                                            html = `<span class='eventColorIndicator' style='background-color:${eventColors.bgColor}'>&nbsp;</span>`;
                                        }
                ...
                $(fullCalendarDiv).on("click", ".fc-day-header", function() {
            ...
            if (!fetchingAgendaEvents) {
                if (target.scrollTop && target.scrollTop > target.scrollHeight - BEFORE_END_Y_BUFFER && globalThis.previousScrollTop < target.scrollTop) {
                    ...
                    if (data.events.length) {
                        ...
                        console.log("first event", data.events.first());
