...
async function initPage(tabName) {
	...
	if (!$("#" + tabName + "Page").length) {
		...
		if (tabName == "welcome") {
			...
			$("#showEventTimeOnBadge, #showEventTimeOnBadgeOptions paper-checkbox, #showDayOnBadge, #showDayOnBadgeOptions paper-checkbox, #excludeRecurringEventsButtonIcon, #excludeHiddenCalendarsFromButton, #showButtonTooltip, #showBusyEvents").change(async function () {
                await waitForStorageSync();
                await sendMessageToBG("checkEvents", { ignoreNotifications: true });
			});
